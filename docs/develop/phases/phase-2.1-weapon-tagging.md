...
### 검증 방법
- STAR 구조가 잘 작성된 경험 → 적절한 무기 분류 결과 반환
- "동아리에서 축제 부스 운영 중 팀원 이탈" → W01(위기극복) 주 무기
- "독학으로 프로그래밍을 배워 앱 개발" → W07(성장/학습) 또는 W04(도전정신)
- 짧은 경험 (50자 미만) → 400 에러 + 안내 메시지
- 프롬프트 DB에서 로드 확인 (하드코딩 아님)
- `experience_weapons` 테이블에 결과 저장 확인
- Supabase Dashboard에서 `prompt_templates.usage_count` 증가 확인
...
### 검증 방법
- 새 경험 등록 → 상세 페이지 이동 → "AI 분석 중..." → 3~5초 후 무기 배지 표시
- 경험 수정 (STAR 변경) → 재태깅 트리거 → 새 결과 반영
- 경험 수정 (제목만 변경) → 재태깅 미발생
- 사용자가 확정한 태그 → 재태깅 시에도 유지
- "AI 재분석" 클릭 → 강제 재태깅 → 결과 업데이트
- 태깅 실패 → 에러 표시 → "다시 시도" → 재실행
...
### 검증 방법
- AI 태깅 후 → 7가지 색상 중 해당하는 무기 배지 정상 표시
- 주 무기와 부 무기 시각적 구분 가능
- "무기 편집" 클릭 → 모달 열림 → 7대 무기 표시
- 대분류 클릭 → 소분류 4개 펼침
- 무기 선택 변경 → 저장 → 배지 업데이트
- `user_modified=true` 확인 (Supabase Dashboard)
- "이 분류가 맞아요" 클릭 → `user_confirmed=true` 확인
...
### 검증 방법
- 경험 목록 상단에 8개 탭 (전체 + 7 무기) 정상 표시
- 각 탭에 해당 무기 색상 적용
- "W01 위기극복" 탭 클릭 → W01 태깅된 경험만 필터링
- 필터 후 "전체" 클릭 → 모든 경험 다시 표시
- 탭 괄호 안 숫자가 실제 경험 수와 일치
- 필터 + 정렬 동시 적용 정상 동작
- URL에 `?weapon=W01` 반영 → 새로고침 후에도 필터 유지
...
### 테스트
- [ ] `npm run build` → 빌드 성공
- [ ] AI 태깅 API 수동 테스트 (다양한 경험 입력)
  - [ ] 위기극복 경험 → W01 분류 확인
  - [ ] 리더십 경험 → W02 분류 확인
  - [ ] 복합 경험 → 주 무기 + 부 무기 2개 분류 확인
- [ ] 자동 트리거 플로우 테스트 (등록 → 태깅 → 표시)
- [ ] 수동 편집 → 저장 → `user_modified=true` 확인
- [ ] 필터 탭 클릭 → 목록 필터링 정상
