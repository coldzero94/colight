...
export function KanbanColumn({ config, applications }: KanbanColumnProps) {
  return (
    <div className="flex w-[280px] shrink-0 flex-col rounded-lg bg-gray-50 lg:w-auto lg:flex-1">
        ...
        {(provided, snapshot) => (
          ...
          >
            ...
            {applications.length === 0 && !snapshot.isDraggingOver && (
              <div className="flex h-full min-h-[80px] items-center justify-center rounded-lg border-2 border-dashed border-gray-200">
                <p className="text-xs text-gray-400">여기에 드래그하세요</p>
              </div>
            )}
          </div>
        )}
