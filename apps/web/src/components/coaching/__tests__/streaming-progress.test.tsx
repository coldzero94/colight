...
describe("StreamingProgress", () => {
  ...
  it("shows red text and 초과 when over limit", () => {
    ...
    const element = container.querySelector('[class*="text-red-400"]');
  ...
  it("shows normal color when under limit", () => {
    ...
    // The count element should use text-foreground, not red
    expect(screen.getByText("500 / 1000자")).toHaveClass("text-foreground");
