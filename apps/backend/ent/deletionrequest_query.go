...
func (_q *DeletionRequestQuery) QueryUser() *UserProfileQuery {
...
func (_q *DeletionRequestQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *DeletionRequestQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (_q *DeletionRequestQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *DeletionRequestQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (_q *DeletionRequestQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (_q *DeletionRequestQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *DeletionRequestQuery) WithUser(opts ...func(*UserProfileQuery)) *DeletionRequestQuery {
	query := (&UserProfileClient{config: _q.config}).Query()
...
//		GroupBy(deletionrequest.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.DeletionRequest.Query().
//		Select(deletionrequest.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (_q *DeletionRequestQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*DeletionRequest, error) {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *DeletionRequest, e *UserProfile) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DeletionRequestQuery) loadUser(ctx context.Context, query *UserProfileQuery, nodes []*DeletionRequest, init func(*DeletionRequest), assign func(*DeletionRequest, *UserProfile)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*DeletionRequest)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (_q *DeletionRequestQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(deletionrequest.Table, deletionrequest.Columns, sqlgraph.NewFieldSpec(deletionrequest.FieldID, field.TypeUUID))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUser != nil {
			_spec.Node.AddColumnOnce(deletionrequest.FieldUserID)
		}
	}
