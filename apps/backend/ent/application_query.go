...
type ApplicationQuery struct {
	...
	withAnalysis         *CompanyAnalysisQuery
...
func (_q *ApplicationQuery) QueryUser() *UserProfileQuery {
...
func (_q *ApplicationQuery) QueryAnalysis() *CompanyAnalysisQuery {
...
func (_q *ApplicationQuery) QueryCoverLetters() *CoverLetterQuery {
...
func (_q *ApplicationQuery) QueryExperienceUsages() *ExperienceUsageQuery {
...
func (_q *ApplicationQuery) WithUser(opts ...func(*UserProfileQuery)) *ApplicationQuery {
	query := (&UserProfileClient{config: _q.config}).Query()
...
func (_q *ApplicationQuery) WithAnalysis(opts ...func(*CompanyAnalysisQuery)) *ApplicationQuery {
	query := (&CompanyAnalysisClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ApplicationQuery) WithCoverLetters(opts ...func(*CoverLetterQuery)) *ApplicationQuery {
	query := (&CoverLetterClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ApplicationQuery) WithExperienceUsages(opts ...func(*ExperienceUsageQuery)) *ApplicationQuery {
	query := (&ExperienceUsageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Application, error) {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *Application, e *UserProfile) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAnalysis; query != nil {
		if err := _q.loadAnalysis(ctx, query, nodes, nil,
			func(n *Application, e *CompanyAnalysis) { n.Edges.Analysis = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCoverLetters; query != nil {
		if err := _q.loadCoverLetters(ctx, query, nodes,
			func(n *Application) { n.Edges.CoverLetters = []*CoverLetter{} },
			func(n *Application, e *CoverLetter) { n.Edges.CoverLetters = append(n.Edges.CoverLetters, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withExperienceUsages; query != nil {
		if err := _q.loadExperienceUsages(ctx, query, nodes,
			func(n *Application) { n.Edges.ExperienceUsages = []*ExperienceUsage{} },
			func(n *Application, e *ExperienceUsage) {
...
func (_q *ApplicationQuery) loadUser(ctx context.Context, query *UserProfileQuery, nodes []*Application, init func(*Application), assign func(*Application, *UserProfile)) error {
...
func (_q *ApplicationQuery) loadAnalysis(ctx context.Context, query *CompanyAnalysisQuery, nodes []*Application, init func(*Application), assign func(*Application, *CompanyAnalysis)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Application)
	for i := range nodes {
		if nodes[i].company_analysis_applications == nil {
			continue
		}
		fk := *nodes[i].company_analysis_applications
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(companyanalysis.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "company_analysis_applications" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ApplicationQuery) loadCoverLetters(ctx context.Context, query *CoverLetterQuery, nodes []*Application, init func(*Application), assign func(*Application, *CoverLetter)) error {
	...
	query.Where(predicate.CoverLetter(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(application.CoverLettersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.application_cover_letters
		if fk == nil {
			return fmt.Errorf(`foreign-key "application_cover_letters" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "application_cover_letters" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *ApplicationQuery) loadExperienceUsages(ctx context.Context, query *ExperienceUsageQuery, nodes []*Application, init func(*Application), assign func(*Application, *ExperienceUsage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Application)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ExperienceUsage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(application.ExperienceUsagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.application_experience_usages
		if fk == nil {
			return fmt.Errorf(`foreign-key "application_experience_usages" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "application_experience_usages" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ApplicationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUser != nil {
			_spec.Node.AddColumnOnce(application.FieldUserID)
		}
	}
